...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTaskEasyVersion() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        long longTime = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(n1 -> n1.getTasks().stream())
                .map(n -> LocalDate.now().getDayOfYear() - n.getCreated().getDayOfYear())
                .reduce(0, (a , b) -> a = a + b);

        long longTask = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .mapToLong(n1 -> n1.getTasks().size())
                .sum();

        System.out.println("Average days of InProgress Tasks is: " + longTime/longTask);

        Assert.assertEquals(10, longTime/longTask);
    }

    @Test
    public void testAddTaskListAverageWorkingOnTaskDifficultVersion() {

        Board project = prepareTestData();

        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        List<Integer> tempList = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(n1 -> n1.getTasks().stream())
                .map(n -> LocalDate.now().getDayOfYear() - n.getCreated().getDayOfYear())
                .collect(toList());
       ...
       double average =  IntStream.range(0, tempList.size())
        ...
        System.out.println("Average days of InProgress Tasks is: " + average);
